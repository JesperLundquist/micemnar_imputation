...
for (i in 1:sim){ #  sim är antal iterationer
  ...
  #Bortfall ganjali
  #kumulativ normalfördelning genererar Pr(Z<y+0.75)
  datagan$ganjali <- pnorm(0.75+datagan$y) 
...
tabell30<- result[,c("Metod","rho", "beta", "PB", "AW", "St.e", "RMSE","CR")]%>%
...
#Spara tabell
write.table(tabell30, file = "tabell30.txt", sep = ",", quote = FALSE, row.names = F)

#Sparar resultat i csv-filer#
write.csv(tabell30, file="tabell30.csv")
