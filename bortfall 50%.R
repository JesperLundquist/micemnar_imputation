...
for (i in 1:sim){ #  sim är antal iterationer
  ...
  #Bortfall ganjali
  #kumulativ normalfördelning genererar Pr(Z<y)
  datagan$ganjali <- pnorm(datagan$y) 
...
tabell50<- result[,c("Metod","rho", "beta", "PB", "AW", "St.e", "RMSE","CR")]%>%
...
#Spara tabell
write.table(tabell50, file = "tabell50.txt", sep = ",", quote = FALSE, row.names = F)

#Sparar resultat i csv-filer#
write.csv(tabell50, file="tabell50.csv")
